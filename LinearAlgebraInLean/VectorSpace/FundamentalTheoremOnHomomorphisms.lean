...
def InducedMap (Φ: LinearMap V W) : LinearMap (V / (Kernel.kernel Φ)) W := by
  ...
  let mk x := Quotient.mk (QuotientSpace.rel V (Kernel.kernel Φ)) x
...
theorem inducedMapIsInjective (Φ: LinearMap V W) : Function.Injective (InducedMap Φ) := by
  ...
  unfold Setoid.r QuotientSpace.rel
  unfold QuotientSpace.relation
  simp
  assumption
